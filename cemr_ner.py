...
from utils import cher_ner_metric
...
if __name__ == '__main__':
    ...
    cehr_ner = cEHRNER(dataset="dataset/CEMRNER", embed_type="bert",
                       dense_layer_type="gru",
                       model_name_or_path="bert-base-chinese",
                       bert_lr=5e-5, crf_lr=1e-3, output_dir="results/cehr_ner",
                       per_gpu_batch_size=8, patience=5, max_epochs=200)

    cehr_ner.train()
    train_time = time.time() - start_time
    cehr_ner.export_results(cehr_ner.dev_path)
    cehr_ner.evaluation(cehr_ner.dev_path, start_time, train_time)
    train_time = time.time() - start_time
    cehr_ner.export_results(cehr_ner.test_path)
    cehr_ner.evaluation(cehr_ner.test_path, start_time, train_time)
